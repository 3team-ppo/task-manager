...
- **Функциональные требования**
  ...
  *Пользователь должен иметь возможность…*
    ...
    3. Создать проект, передав название проекта и никнейм пользователя-создателя проекта.
        1. В каждом проекте создается статус по умолчанию "CREATED".
    4. Создать задачу в проекте, передав название проекта и название задачи. Задача создается в статусе по умолчанию "CREATED".
    ...
    7. Изменить статус задачи, передав ее название и новый статус (пользователь может выбрать один из доступных статусов проекта, к которому относится задача).
    ...
    10. Создать новый статус, передав ID проекта, название статуса и цвет для отображения в UI.
    11. Удалить статус, если он не присвоен ни одной задаче.
...
| Проект (Project) | Логическая сущность, которая объединяет в себе некоторое количество *участников* проекта (они являются *пользователями*), которые могут создавать *задачи внутри* этого проекта, а также задачи проекта. Соответственно, только участники проекта могут просматривать его содержимое и изменять его. | - ID <br/>- Имя <br/>- Создатель <br/>- Дата создания <br/>- Дата последнего изменения <br/>- Участники                                                          | - Участник <br/>- Создатель  <br/> - Задача <br/>- Статус <br/><br/> В проекте есть участники и задачи, которые относятся к данному проекту                                                         | - Создание (пользователь → *создает* → проект).  <br/><br/>- Изменение названия (пользователь создатель → *изменяет название* → проекта).  <br/><br/>- Удаление (пользователь создатель → *удаляет* → проект).  <br/><br/>- Просмотр (участник проекта → *просматривает* → проект).  <br/><br/>- Добавление участников (участник проекта → *добавляет участников* → в проект). <br/><br/>- Добавление статуса (участник проекта → *добавляет статус* → в проект).                                 |
| Задача(Task) | Логическая сущность, содержащая информацию о конкретной задаче в рамках проекта. Содержит информацию для отслеживания выполнения данной задачи.  | - ID <br/>- Имя <br/>- Статус <br/>- Приоритет <br/>- Создатель <br/>- Дата создания <br/>- Дата последнего изменения <br/>- Оценка по времени <br/>-Исполнители | - Исполнитель <br/>- Создатель <br/>- Проект <br/>- Статус <br/><br/>Задача относится к определенному проекту. Исполнители относятся к определенной задаче.                                         | \- Создание (участник проекта → *создает* → задачу).  <br/><br/>- Изменение (участник проекта → *изменяет* → задачу).  <br/><br/>- Удаление (участник проекта → *удаляет* → задачу).  <br/><br/>- Просмотр (участник проекта → *просматривает* → задачу).  <br/><br/>- Назначение исполнителя (участник проекта → *назначает исполнителя* → задачи). <br/><br/>- Изменение статуса (участник проекта → *изменяет статус* → задачи).                                                                                                                                             |
...
| Статус (Status)       | Отражение стадии выполнения задачи.                                                                                                                                                                                                                                                                  | - ID <br/>- Название<br/> - Цвет отображения на UI                                                                                                               | - Задача <br/>- Проект <br/><br/>Задача создается в рамках проекта и относится к нескольким задачам.                                                                                                | - Создание (участник проекта → *создает* → статус).  <br/><br/>- Удаление (участник проекта → *удаляет* → статус).  <br/><br/>- Просмотр (участник проекта → *просматривает* → статусы).  <br/><br/>- Изменение статуса задачи (участник проекта → *изменяет в задаче* → статус).                                                                                                                                                                                                                 |
...
| Контекст Статусов (Status Context)                     | Этот контекст отвечает за управление статусами в рамках проекта, включая их создание и изменение.                                       | - Статус  <br/>- Создание статуса  <br/>- Изменение статуса <br/>- Удаление статуса   <br/>- Изменение статуса задачи                                                                                   |
...
**Пользователь (User)**  
Пользователь — это субъект, взаимодействующий с системой. Каждый пользователь имеет уникальные параметры, которые позволяют системе идентифицировать его и предоставить доступ к системе.
...
**Проект (Project)**  
Проект представляет собой коллективное пространство, объединяющее несколько пользователей для работы над задачами. Каждый проект включает определенный набор задач и участников
...
**Задача(Task)**  
Задача — это единица работы, требующая выполнения в рамках проекта. Каждая задача имеет свои параметры, такие как статус и приоритет, которые управляют ее выполнением и отслеживанием.
...
**Статус(Status)**  
Статус — это стадия выполнения задачи. На UI отображается как колонка с названием и окрашенная в определенный цвет.

Контекст Статусов (Status Context)

Атрибуты:
- ID
- Название
- Цвет отображения на UI
- Id проекта
...
**Анализ функциональных требований:**
  ...
  5. Создать задачу в проекте, передав ID проекта и название задачи. Задача создается в статусе “не выполнено”
     ...
     2. **Требуемые изменения:** Создание новой записи задачи, связанной с проектом, с присвоением статуса "CREATED". Обновление списка задач в проекте.
  ...
  6. Изменять список пользователя(-ей) исполнителей задачи: добавить или удалить исполнителя, передав ID задачи и ID пользователя(-ей), если задача в статусе “CREATED”
  ...
  8. Изменить статус задачи, передав ее id и новый статус
     ...
     1. **Сущности:** Задача (Task), Проект (Project), Статус (Status)

     2. **Требуемые изменения:** Обновление статуса задачи на новый.

     3. **Контекст:** Контекст управления задачами (Task Management Context), Контекст проектов (Project Context), Контекст статусов (Status Context)
  ...
  11. Создать статус в проекте, передав ID проекта, название статуса и его цвет.

      1. **Сущности:** Статус (Status)

      2. **Требуемые изменения:** Добавление нового статуса с названием и цветом.

      3. **Контекст:** Контекст статусов (Status Context)

      4. **Инварианты:** Статус должен быть привязана к существующему проекту. Проект должен содержать обновленный список статусов с добавленным новым статусом.

      5. **Требуется ли транзакционность:** Да, транзакционность требуется. Необходимо, чтобы статус и обновленный список статусов проекта сохранялись одновременно. Это предотвратит ситуацию, когда статус создается, но не добавляется в проект, или проект обновляется, но статус не создается.

      6. **Строгая согласованность:** Создание статуса и обновление списка статусов в проекте должны происходить одновременно.

  12. Удалить статус, если он не присвоен ни одной задаче.

      1. **Сущности:** Статус (Status)

      2. **Требуемые изменения:** Удаление статуса, если он не присвоен ни одной задаче.

      3. **Контекст:** Контекст статусов (Status Context)

      4. **Инварианты:** Статус не должен быть присвоен ни одной задаче. Проект должен содержать обновленный список статусов без удаленного статуса.

      5. **Требуется ли транзакционность:** Да, транзакционность требуется. Необходимо, чтобы статус удалялся и обновленный список статусов проекта сохранялся одновременно. Это предотвратит ситуацию, когда статус удаляется, но не удаляется, или проект обновляется, но статус не удаляется.

      6. **Строгая согласованность:** Удаление статуса и обновление списка статусов в проекте должны происходить одновременно.
...
3. Агрегат "Задача" (Task Aggregate)
     ...
     3. **Описание:** Управляет созданием и изменением задач в проекте, включая список исполнителей и статусы задач. Обновление названия задачи, изменение статуса и списка исполнителей будут происходить в рамках этого агрегата. Обеспечивает атомарные операции по добавлению исполнителей и проверке их статуса. Привязан к проекту через внешний ключ (ID проекта).
4. Агрегат "Статус" (Status Aggregate)
    1. **Сущности:** Cтатус (Status)
    2. **Контекст:** Контекст статусов (Status Context)
    3. **Описание:** Управляет созданием и изменением cтатусов в проекте. Привязан к проекту через внешний ключ (ID проекта).

## Lab 7
**Операции чтения:**
1. Получить информацию о проекте (по Project.Id)
   1. **Ограниченные контексты:** Контекст Проектов
   2. **Агрегаты:** Агрегат "Проект"
2. Список всех проектов пользователя (по User.Id)
   1. **Ограниченные контексты:** Контекст Проектов
   2. **Агрегаты:** Агрегат "Проект"
3. Получить информацию о задаче (по Task.Id)
   1. **Ограниченные контексты:** Контекст Задач
   2. **Агрегаты:** Агрегат "Задача"
4. Список всех задач в проекте (по Project.Id, с фильтром по статусу)
   1. **Ограниченные контексты:** Контекст Задач, Контекст Проектов
   2. **Агрегаты:** Агрегат "Задача", Агрегат "Проект"
5. Получить информацию о задаче (по User.Id исполнителя)
    1. **Ограниченные контексты:** Контекст Задач
    2. **Агрегаты:** Агрегат "Задача"
6. Список всех статусов проекта (по Project.Id)
   1. **Ограниченные контексты:** Контекст Статусов, Контекст Проектов
   2. **Агрегаты:** Агрегат "Статус", Агрегат "Проект"
7. Проверка существования пользователя (по никнейму)
   1. **Ограниченные контексты:** Контекст Пользователей
   2. **Агрегаты:** Агрегат "Пользователь"
8. Поиск пользователя (по никнейму)
   1. **Ограниченные контексты:** Контекст Пользователей
   2. **Агрегаты:** Агрегат "Пользователь"
...
1. Проекция проектов:
   1. Структура данных:
       - Хранение проектов в виде таблицы в реляционной базе данных, где ключом будет Project.Id и будут содержаться остальные актуальные поля данного проекта, а также задача будет хранится в виде сущности с идентификатором, именем и статусом, также можно создать индексы для оптимизированного поиска по Project.Id и User.Id(создатель)
   2. Способы построения:
       - При создании нового проекта добавлять запись в проекцию
       - При изменении данных проекта обновлять соответствующие записи
   3. Можно применить для операций 1, 2, 4 для ускоренного поиска по идентификаторам, а также удобного получения сводной информации по задачам для получения их в виде списка
2. Проекция задач:
    1. Структура данных:
        - Хранение задач в виде таблицы в реляционной базе данных, где ключом будет Task.Id и будут содержаться остальные актуальные поля данной задачи, также можно создать индекс для оптимизированного поиска по Task.Id и User.Id(исполнитель)
    2. Способы построения:
        - При создании новой задачи добавлять запись в проекцию
        - При изменении задачи обновлять соответствующие записи
    3. Можно применить для операций 3, 5 для ускоренного поиска по идентификаторам
3. Проекция пользователей:
    1. Структура данных:
        - Хранение пользователей в виде таблицы в реляционной базе данных, где ключом будет User.Id и будут содержаться остальные актуальные поля данного пользователя, также можно создать индекс для оптимизированного поиска по User.Id
    2. Способы построения:
        - При создании нового пользователя добавлять запись в проекцию
        - При изменении пользователя обновлять соответствующие записи
    3. Можно применить для операций 7, 8 для ускоренного поиска по идентификатору
4. Проекция статусов:
    1. Структура данных:
        - Хранение статусов проектов в виде массива или списка статусов, привязанных к каждому проекту в реляционной базе данных. Ключом является Project.Id, по данному полю можно создать индекс, статусы, так как не содержат много информации можно хранить в списке как сущности, содержащие полную информацию.
    2. Способы построения:
        - При создании нового статуса добавлять запись в проекцию
        - При изменении статуса обновлять соответствующие записи
    3. Можно применить для операций 6 для ускоренного поиска по идентификаторам