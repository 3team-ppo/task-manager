...
1. Проекция проектов:
    ...
    3. Можно применить для операций 1, 2, 4, 6 для ускоренного поиска по идентификаторам, а также удобного получения сводной информации по задачам для получения их в виде списка
2. Проекция задач:
    1. Структура данных:
        - Хранение задач в виде таблицы в документоориентированной базе данных mongoDB, где ключом будет Task.Id и будут содержаться остальные актуальные поля данной задачи, также можно создать индекс для оптимизированного поиска по Task.Id и User.Id(исполнитель)
    2. Способы построения:
...
3. Проекция пользователей:
    1. Структура данных:
        - Хранение пользователей в виде таблицы в документоориентированной базе данных mongoDB, где ключом будет User.Id и будут содержаться остальные актуальные поля данного пользователя, также можно создать индекс для оптимизированного поиска по User.Id
    2. Способы построения:
